...
typedef std::pair<double,CvPoint> DP;
struct DPComparator
{
	bool operator()(const DP & p, const DP & q)
		{
...
void
MapImage::fillSmallHolesInFreeSpaceAroundRobot(CvPoint & robotPos, unsigned int fillLimit)
{
	CvPoint offset[24];
	int k = 0;
	for(int i = -2; i <=2; i++)
	...
	int b_l = robotPos.x - fillLimit;
	int b_r = robotPos.x + fillLimit;
	int b_b = robotPos.y - fillLimit;
	int b_t = robotPos.y + fillLimit;
	b_l = (b_l < 0) ? 0 : b_l;
	b_r = (b_r >= img->widthStep) ? img->widthStep-1 : b_r;
	b_b = (b_b < 0) ? 0 : b_b;
	b_t = (b_t >= img->height) ? img->height-1 : b_t;

	bool marked[b_r-b_l+1][b_t-b_b+1];
	for(int i = 0; i < (b_r-b_l+1); i++)
		for(int j = 0; j < (b_t-b_b+1); j++)
			marked[i][j] = false;
	
	std::priority_queue<std::pair<double,CvPoint>,
						std::vector<std::pair<double,CvPoint> >,
						DPComparator > q;  // prority queue used to order the points accordint to their shortest distances
	q.push(std::pair<double,CvPoint>(0.0,robotPos));
	marked[robotPos.x-b_l][robotPos.y-b_b] = true;
	while( !q.empty() )
	{
		std::pair<double, CvPoint> heapval = q.top(); q.pop();
		CvPoint p = heapval.second;
		double dis = heapval.first;
		int mappedCount = 0;
		for(int k = 0; k < 24; k++)
		{
			CvPoint newP; newP.x = p.x + offset[k].x;	newP.y = p.y + offset[k].y;   // neighboring point
			if(newP.x < 0 || newP.x > img->widthStep-1 || newP.y < 0 || newP.y > img->height-1)   // boundary check for whole image
				continue;
			if( !IS_UNKNOWN( static_cast<unsigned int>(img->imageData[newP.y*(img->widthStep)+newP.x])) )  // if already mapped
				mappedCount++;
			if(marked[newP.x-b_l][newP.y-b_b] || newP.x < b_l || newP.x > b_r || newP.y < b_b || newP.y > b_t) // if marked( meaning added to queue)
				continue;   // or outside window, don't bother
			double newdis = -sqrt( (p.x-newP.x)*(p.x-newP.x) + (p.y-newP.y)*(p.y-newP.y) ) + dis;
			q.push(std::pair<double,CvPoint>(newdis, newP));  // else add the neighbor point to queue
			marked[newP.x-b_l][newP.y-b_b] = true;  // mark as added
		}

		if( IS_UNKNOWN( static_cast<unsigned int>(img->imageData[p.y*(img->widthStep) + p.x]) ) && mappedCount >= 19 )
		{
			img->imageData[p.y*img->widthStep + p.x] = 0; // make it free
		}		
	}
}
